...
void loop() {
  ...
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 2000 || sendDataPrevMillis == 0)) {
    ...
    while (Serial.available()) {
      ...
      char seat_num = incomingData[incomingData.length()-3];
      char sender_num = incomingData[incomingData.length()-2];
      incomingData[incomingData.length()-1] = '\0';
      incomingData[incomingData.length()-2] = '\0';
      incomingData[incomingData.length()-3] = '\0';
      ...
      else if(incomingData.length() == 11)
      {
